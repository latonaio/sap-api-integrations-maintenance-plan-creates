...
import (
	...
	sap_api_output_formatter "sap-api-integrations-maintenance-plan-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callMaintenancePlanSrvAPIRequirementHeader("A_MaintenancePlan", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaintenancePlanSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
	body, err := json.Marshal(header)
	if err != nil {
		return nil, xerrors.Errorf("API request error: %w", err)
	}
	url := strings.Join([]string{c.baseURL, "API_MAINTENANCE_PLAN_SRV", api}, "/")
